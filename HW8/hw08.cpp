...
int main()
{
    ...
    //double *q = p; 	// [1.2] variable q of type pointer to double is points to x
    //			// This cannot be done as a double may not point to an int
    x = *p; 		// [1.3] update the value of p to 5, use * to assign to x through p
    ...
    //double& s = &y;	// [1.9] variable s of type reference to double is refers to y
    //			    This cannot be done as a double cannot point to an int
    ...
    r2 = y;		// [1.13] assign r to r2, the value of y is assigned to y2 
    //r = r2;		// [1.14] assign y2 to r, r2 and r both point to y2
    //			// Reference values may not be changed
    ...
    *pd = 3;
    ...
    //pd += 1000;    		// [2.3] This will not work as it will increment the address of the pointer by 1000
    //double d = *pd;		// [2.4] pd points to an array, therefoer you must specify an index 
    //*pd = 12.34;   		// [2.5] Again, an index must be specified in order to modify an array 

    // ... // [2.6] compute the number of elements between pd and pd2 by taking the difference between pd2 and pd
    //int dif2 = pd + pd2; // [2.7] // Will not work as pd and pd2 unreferenced are address values
    //int dif3 = pd2 - pd3;// [2.8] // Will not work ad pd2 and pd3 are adressed when unreferenced
    ...
    for (double* p = &aDoubles[0]; *p <= aDoubles[9]; p++) cout << *p << '\n';
    cout << endl;
    ...
    for (double* p = &aDoubles[9]; *p >= aDoubles[0]; p-- ) cout << *p << '\n';
    cout << endl;
    ...
    // [2.12] pd6 and pd7 are pointing to different arrays
	...
	// Q#4 - new, delete operator examples
    {
        ...
        //pi = pd;  // [4.5] pi points to pd
                    // cannot point an int pointer to a double pointer
        //pd = pi;  // [4.6] pd points to pi
                    // cannot point a double pointer to an int pointer 
        ...
        delete pi;            	// [4.7] how are the values of ri, ri2, ri3 affected by delete statement?
				// ri is pointing to pi which will be deallocated from the heap, ri2 and ri3 are pointing to something else	
        delete [] ri3;        	// [4.8] how are the values of ri, ri2, ri3 affected by delete statement?
				// ri2 and ri3 will be deallocated from the heap as qi is, ri is unaffected
